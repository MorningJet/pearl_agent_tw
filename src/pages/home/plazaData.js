...
function listMasterPublished() {
  return (plazaMaster.designs || [])
    ...
    .map((d) => {
      // Prefer shared remote count when available so all users see the same number.
      const remote = getRemotePlazaDesign(d.id)
      const useCount = remote
        ? Number(remote.useCount) || 0
        : getPlazaUseCount(d.id, Number(d.use_count) || 0)
      return {
        id: d.id,
        title: d.title,
        tags: d.blurb || '',
...
export function getSeedPlazaAsPublished(id) {
  ...
  return {
    ...
    useCount: remote ? Number(remote.useCount) || 0 : getPlazaUseCount(d.id, baseCount),
    beads,
...
function publishedToPlaceholder(p) {
  ...
  // Remote entries already carry shared useCount; for local-only prefer remote patch if any.
  const remote = getRemotePlazaDesign(p.id)
  const useCount = remote ? Number(remote.useCount) || 0 : p.useCount || 0
